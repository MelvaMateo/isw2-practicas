...
test('calcula total sin descuento cuando el subtotal es menor a 500', () => {
    const inv = { p1: 10 };
    const resultado = procVta([{ id: 'p1', cant: 2, precio: 100 }], 'normal', inv);
...
test('aplica 10% de descuento cuando el subtotal supera 500', () => {
    const inv = { p1: 10 };
    const resultado = procVta([{ id: 'p1', cant: 6, precio: 100 }], 'normal', inv);
...
test('retorna null si la cantidad es 0 o negativa', () => {
    const inv = { p1: 10 };
    const resultado = procVta([{ id: 'p1', cant: 0, precio: 100 }], 'normal', inv);
...
test('retorna null si no hay stock suficiente', () => {
    const inv = { p1: 1 };
    const resultado = procVta([{ id: 'p1', cant: 5, precio: 100 }], 'normal', inv);
    assertEqual(resultado, null);
});
...
test('agrega "Total mayorista" al recibo cuando el cliente es mayorista', () => {
    const inv = { p1: 10 };
    const resultado = procVta([{ id: 'p1', cant: 2, precio: 100 }], 'mayorista', inv);
...
test('descuenta del inventario la cantidad vendida', () => {
    const inv = { p1: 10 };
    procVta([{ id: 'p1', cant: 6, precio: 100 }], 'normal', inv);
